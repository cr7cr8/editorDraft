    ...
    conditionWrap({
      ...
      metaArr.forEach(function (item, index) {
        ...
        if (styleArr && styleArr.length > 0) {
          if (styleArr.includes("linkTagOn") || styleArr.includes("linkTagOff")) {
            ...
            newContent = _draftJs.Modifier.removeInlineStyle(newContent, newSelection, "linkTagOn");
            newContent = _draftJs.Modifier.removeInlineStyle(newContent, newSelection, "linkTagOff");
          }
        }
      });
      var matchArr;
      ...
      while ((matchArr = regx.exec(blockText)) !== null) {
        ...
        if (mentionOn) {
          ...
          newContent = _draftJs.Modifier.applyInlineStyle(newContent, newSelection, "linkTagOn");
        } else if (mentionOff) {
          tagStartPos = start;
          tagEndPos = end;
          newSelection = newSelection.merge({
          ...
          newContent = _draftJs.Modifier.applyInlineStyle(newContent, newSelection, "linkTagOff");
        }
      }
    });
